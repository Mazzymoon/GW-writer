...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="国企公文多 Agent 智能写作系统 CLI")
    subparsers = parser.add_subparsers(dest="command")

    build = subparsers.add_parser("build", help="构建公文知识检索索引")
    build.add_argument("--docs", default="./docs", help="文档目录，支持 .pdf/.md/.txt，默认 ./docs")

    inspect = subparsers.add_parser("inspect", help="查看知识检索工具召回结果")
    ...
    draft = subparsers.add_parser("draft", help="运行多 Agent 公文写作流程")
    ...
    draft.add_argument("--disable-memory", action="store_true", help="本次运行不启用长期成功案例记忆")
...
def command_draft(args: argparse.Namespace, settings: Settings) -> int:
    ...
    retriever = HybridRetriever(kb, reranker)
    case_memory = None
    if settings.enable_long_term_memory and not args.disable_memory:
        case_memory = CaseMemory(settings)
...
def print_workflow_result(result: WorkflowResult) -> None:
    ...
    if result.review.suggestions:
        ...
        for item in result.review.suggestions:
            print(f"- {item}")
    if result.memory_cases:
        print("\n=== 历史成功案例参考 ===")
        for index, case in enumerate(result.memory_cases, start=1):
            preview = case.replace("\n", " ")
            print(f"[{index}] {preview[:500]}{'...' if len(preview) > 500 else ''}")
    print("\n=== 参考依据 ===")
...
def workflow_to_dict(result: WorkflowResult) -> dict:
    return {
        ...
        "memory_cases": result.memory_cases,
    }
